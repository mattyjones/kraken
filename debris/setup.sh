...
# New hostname. If you don't want to change it, then just leave this blank.
# If you change the name, the system will require a reboot.
local_hostname="host_name"

# If you want the box to reboot automatically at the end of the buildout,
# set this to true
auto_reboot=false

# If you want to change the default password, set this to true, but it
...
change_hostname() {
  ...
  if [[ $local_hostname == "" ]]; then
    echo "Using current hostname"
  else
    sed -i 's/kali/kali-test/' /etc/hostname
  fi
...
main() {
  ...
  # TODO chack to make sure the version < 1.0.6
  install_masscan
  ...
  #install -v -g "$logname" -o "$logname"  -d "/mnt/$share_dir/VM\ Data/dotfiles/." "/home/$logname/"

  # cp -r "/mnt/$share_dir/VM\ Data/dotfiles/." "/home/$logname/"
  ...
  # # Configuring nvim
  # cp -r "/mnt/$share_dir/VM Data/dotfiles/nvim" "/home/$logname/config"
